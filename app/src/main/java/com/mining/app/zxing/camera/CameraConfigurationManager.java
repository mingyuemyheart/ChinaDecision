...
//      cameraResolution = findBestPreviewSizeValue(previewSizeValueString, screenResolution);
      ...
      for (int i = 0; i < preList.size(); i++) {
        if (Double.valueOf(preList.get(i).width)/Double.valueOf(preList.get(i).height) == Double.valueOf(displayH)/Double.valueOf(displayW)) {
      ...
      if (preWidth > 0 && preHeight > 0) {
        cameraResolution = new Point(preWidth, preHeight);
//        parameters.setPreviewSize(preWidth, preHeight);// 设置预览照片的大小
      }else {
        cameraResolution = new Point(preList.get(0).width, preList.get(0).height);
//        parameters.setPreviewSize(preList.get(0).width, preList.get(0).height);// 设置预览照片的大小
      }

      Log.e("cameraResolution", cameraResolution.x+","+cameraResolution.y);
