...
import com.china.R;
import com.china.adapter.TyphoonNameAdapter;
import com.china.adapter.TyphoonYearAdapter;
...
public class TyphoonRouteActivity extends BaseActivity implements OnClickListener, OnMapClickListener, AMapLocationListener, GeocodeSearch.OnGeocodeSearchListener,
		OnMarkerClickListener, InfoWindowAdapter, AMap.OnMapScreenShotListener, CaiyunManager.RadarListener, OnCameraChangeListener{
	...
	//	private List<TyphoonDto> points = new ArrayList<>();//某个台风的数据点
//	private List<TyphoonDto> forePoints = new ArrayList<>();//预报的点数据
	 ...
	 * 获取某一年的台风列表信息
	 */
	private void OkHttpTyphoonList(String url, final int currentYear, final int selectYear) {
...
//		}
	 ...
	 * 获取分钟级降水图
	 */
	private void OkHttpMinute(String url) {
		OkHttpUtil.enqueue(new Request.Builder().url(url).build(), new Callback() {
			@Override
			public void onFailure(Call call, IOException e) {
...
//				}
	 ...
	 * 获取云图数据
	 */
	private void OkHttpCloudChart(String url) {
		OkHttpUtil.enqueue(new Request.Builder().url(url).build(), new Callback() {
			@Override
			public void onFailure(Call call, IOException e) {
...
//		LatLng rightLatlng = aMap.getProjection().fromScreenLocation(rightPoint);
	...
	private void OkHttpWind(String url) {
		OkHttpUtil.enqueue(new Request.Builder().url(getWindSecretUrl(url, "1000")).build(), new Callback() {
			@Override
			public void onFailure(Call call, IOException e) {
...
//		showWind(waitWindView);
	...
	public void onMapScreenShot(final Bitmap bitmap1) {//bitmap1为地图截屏
		...
		Bitmap bitmap3 = CommonUtil.mergeBitmap(TyphoonRouteActivity.this, bitmap1, bitmap2, true);
...
//		Bitmap bitmap4 = BitmapFactory.decodeStream(getClass().getResourceAsStream("/res/drawable-hdpi/iv_share_bottom.png"));
		...
		CommonUtil.share(TyphoonRouteActivity.this, bitmap);
	}
		...
		if (v.getId() == R.id.llBack) {
			...
			if (isWindOn == false) {//添加图层
				if (isHaveWindData == false) {
					OkHttpWind("http://scapi.weather.com.cn/weather/getwindmincas");
				}else {
					reloadWind();
				}
				isWindOn = true;
			...
			aMap.getMapScreenShot(TyphoonRouteActivity.this);
